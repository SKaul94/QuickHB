...
export class QuickHbEditor extends LitElement {
    ...
    constructor() {
        ...
        this.database = [];
        this.gender = 'm';
        this.placeholder = 'Text frei eingeben, Vorschläge aus Titel, Shortcut oder ID ...';
    ...
    updated( changedProperties ) {
        if ( changedProperties.has('variables') ) {
            ...
            for ( const quickVariableSpan of quickVariables ){
                ...
                const name = quickVariableSpan.dataset.name;
                const value = this.variables[ name ];
                if ( content !== value ) {
                   quickVariableSpan.innerText = value || '['+name+']';
                }
            }
        }
    }
    ...
    _handleInput( event ) {
        ...
        if ( event.inputType === 'insertText' && event.data === ']' ) {
            // inserted text is the end of a variable, e.g. [X]
            if ( event.target === this.editorElement ) {
                event.target.innerHTML = event.target.innerText.replaceAll(
                    RegexSquareBrackets,      
                    (_, varName) => `<span title="siehe Tabelle [${varName}]" class="quick-variable" data-name="${varName}">${this.variables?.[varName] || '['+varName+']'}</span>\u00A0`
                );
                // 1. Fokus auf das Element setzen
                this.editorElement.focus();

                // 2. Selection und Range erstellen
                const selection = this.shadowRoot.getSelection();
                const range = document.createRange();

                // 3. Den gesamten Inhalt des Elements in die Range aufnehmen
                range.selectNodeContents( this.editorElement );

                // 4. Die Range am Ende zusammenfalten (false = Ende, true = Anfang)
                range.collapse(false);

                // 5. Die alte Auswahl löschen und die neue setzen
        ...
        this._matches = this.database.filter(item => {
            ...
            return t.includes(term) || s.startsWith(term) || item.id.startsWith(term);
        }).slice(0, 16);
    ...
    _insertItem(item) {
        ...
        const spintaxKey = 'spintax_' + gender;
        const rawText = item[ spintaxKey ];
        ...
        try {
            ...
            newNode.classList.add( item.id, spintaxKey );
