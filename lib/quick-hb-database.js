...
export const INDEXEDDB_KEYS = ['spintax', 'person'];
export const INDEXEDDB_SPINTAX_KEY = INDEXEDDB_KEYS[0];
export const INDEXEDDB_PERSON_KEY = INDEXEDDB_KEYS[1];
...
export class QuickHbDatabase extends LitElement {
  ...
  async _load(event) {
    ...
    // store into Idb
    for (const key of INDEXEDDB_KEYS) {
      await Idb.set(key, data[key]);
    }
    this.database = data[INDEXEDDB_SPINTAX_KEY];
    document.getElementById('person-editor').fields = data[INDEXEDDB_PERSON_KEY];
    this.dispatchEvent(new CustomEvent('database-changed', {
      bubbles: true,
      composed: true,
      detail: {
        key: INDEXEDDB_SPINTAX_KEY,
        size: data.length
      }
    }));
  }
  ...
  async _saveData(options) {
    ...
    try {
      ...
      if ('showSaveFilePicker' in window) {
        ...
        // Fallback: use `a[href]` download or `URL.createObjectURL()`
  ...
  async indexedDbData() {
    const dataObject = {};
    for (const key of INDEXEDDB_KEYS) {
      dataObject[key] = await Idb.get(key);
    }
  ...
  async _zip(event) {
    ...
    const arraybuffer = encoder.encode(await this.indexedDbData());
    const encryptedBinary = encrypt(arraybuffer, null, this.shadowRoot);
    const zip = new JSZip();
    zip.file(INDEXEDDB_SPINTAX_KEY, encryptedBinary, { binary: true });
  ...
  async _databaseChanged(event) {
    ...
    await Idb.set(INDEXEDDB_SPINTAX_KEY, this.database);
  ...
  _plusButton(event) {
    ...
    Idb.set(INDEXEDDB_SPINTAX_KEY, this.database);
    this.dispatchEvent(new CustomEvent('database-changed', {
      bubbles: true,
      composed: true,
      detail: {
