...
export class QuickHbDatabase extends LitElement {
  ...
  // Runs after every update (including the first)
  // async updated( changedProperties ) {
  //   if (changedProperties.has('database')) {
  //     // re-index after changed database
  //     this._index = {};
  //     this._shortcuts = new Set();
  //     for ( const rec of this.database ) {
  //       // Duplicate ID
  //       if ( this._index[ rec.id ] ) throw new Error( `Duplicate key ${rec.id}` );
  //       this._index[ rec.id ] = rec;
  //       // Duplicate shortcut
  //       if (this._shortcuts.has(rec.shortcut)) throw new Error( `Duplicate shortcut ${rec.shortcut}.` );
  //       this._shortcuts.add( rec.shortcut );
  //     }
  //     // re-structure after changed database
  //     this._structure = await Idb.get( INDEXEDDB_KEY_STRUCTURE );
  //     if ( ! this._structure ) {
  //         // get list of sections from database
  //         this._structure = [];
  //         for ( const id of Object.keys( this._index ) ) {
  //             const section = this._index[ id ].section;
  //             if ( ! this._structure.includes( section ) ) this._structure.push( section );
  //         }
  //     }
  //   }
  // }
  ...
  render() {
    return html`
      ...
      <button id="plus-button" class="blue-button full-width" @click="${this._plusButton}"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-clipboard-plus" viewBox="0 0 16 16">
