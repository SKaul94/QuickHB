...
export class QuickHbDatabase extends LitElement {
  ...
  /**
   * Build Index over id-s of records in an array
   * and search for duplicates
   * @param {Array} anArray 
   * @returns Index with id-s as keys and records as values
   */
  static objectFromArray( anArray ) {
    const obj = {};
    const seen = new Set();
    for ( const rec of anArray ) {
      // Dopplung shortcut
      if (seen.has(rec.shortcut)) throw new Error( `Duplicate Shortcut ${rec.shortcut}.` );
      seen.add(rec.shortcut);
      // Dopplung ID
      if ( obj[rec.id] ) throw new Error( `Duplicate ID ${rec.id}.` );
      obj[rec.id] = rec;
    }
    return obj;
  }
  ...
  async _load() {
    ...
    if ( Array.isArray( data ) ) {
      this.database = this.constructor.objectFromArray( data );
    } else {
      this.database = data;
    }
    // store only objects into Idb
    await Idb.set( INDEXEDDB_KEY, this.database );
    // const elem = this.shadowRoot.getElementById('message');
    // elem.classList.remove('hidden');
    this.message = 'Datenbank geladen.'; // re-rendering showing message
    setTimeout(_=>{ this.message = '' }, 3000); // triggers re-render
  ...
  render() {
    return html`
      ...
      <div id="record-list">  
        ${Object.values(this.database || {}).map( ( rec ) => html`<quick-hb-record-editor
          .record=${rec}
