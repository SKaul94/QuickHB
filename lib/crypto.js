 ...
 */
async function encrypt(content, password, root) {
  if ( ! password ) password = getPassword( root );
...
function getPassword( root ){
  if ( ! root ) root = document;
  return root.querySelector('.password')?.value || prompt('Passwort?');
}
