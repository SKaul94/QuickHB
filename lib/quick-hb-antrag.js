...
export class QuickHbAntrag extends LitElement {
    ...
    async updated( changedProperties ) {
        ...
        if ( changedProperties.has('database') && this.database ) {
            // get extra info from IndexedDB
            let structure = await Idb.get( INDEXEDDB_KEY_STRUCTURE );
            // ... or use database for deriving the structure
            if ( ! structure ) {
                // get list of sections from database
                structure = [];
                for ( const rec of this.database || [] ) {
                    const section = rec.section;
                    if ( section && ! structure.includes( section ) ) structure.push( section );
                }
            }
            this.structure = structure.length ? structure : ['Antrag'];

            let newKeyFound = false;
            for ( const newKey of this.variablesFromDatabase ) {
                if ( ! this.variables[ newKey ] ) {
                    newKeyFound = true;
                    this.variables[ newKey ] = '';
                }
            }
            if ( newKeyFound ) this.variables = { ...this.variables };
        }
    }
    ...
    get variablesFromDatabase(){
        if ( ! this.database || this.database.length === 0 ) return null;
    ...
    selectDatabase(header) {
        let resultSubset = this.database?.filter(rec => rec.section === header);
    ...
    render() {
        ...
        return html`
            <div id="header">
                ...
                <button id="export" class="blue-button" @click=${this.exportPdf}>Drucken</button>
            </div>
