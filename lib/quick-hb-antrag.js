...
import { INDEXEDDB_SPINTAX_KEY } from './quick-hb-database.js';
...
export class QuickHbAntrag extends LitElement {
    static properties = {
        ...
        _structure: { type: Array, state: true },
    ...
    async firstUpdated() {
        ...
        this.database = await Idb.get(INDEXEDDB_SPINTAX_KEY) || [];
    ...
    async openInDialogue(event) {
        const content = this.compileFinalAntrag();
        const htmlString = this.buildPdfHtml(content);
        const fileName = "antrag.html"
        // 1. Create a File object from your string
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            this.downloadHTML(event);
        }
    }

    async downloadHTML(event) {
        const content = this.compileFinalAntrag();
    ...
    buildPdfHtml(text, namespace = '') {
        const personalienTable = this._renderPersonalienTableHTML();
        return `
            <!doctype html>
            <html ${namespace} lang="de">
            <head>
            <meta charset="utf-8">
            <title>Antrag</title>
            ...
            .personalien-table th, .personalien-table td {
                border: 1px solid #000;
                padding: 4pt 8pt;
                text-align: left;
            }

            .personalien-table th {
    ...
    _renderPersonalienTableHTML() {
        const personEditor = document.getElementById('person-editor');
        if (!personEditor) return '';

        if (!personEditor.data || Object.keys(personEditor.data).length === 0) return '';

        let html = '<table class="personalien-table">\n';
        for (const [key, value] of Object.entries(personEditor.data)) {
            html += `  <tr><th>${this.escapeHtml(key)}</th><td>${this.escapeHtml(value)}</td></tr>\n`;
        }
        html += '</table>';
        return html;
    }
    ...
    /**
     * open window for printing dialogue
     * @param {String} htmlContent - HTML with <html>, head, style, body
     */
    openPrintWindow(htmlContent) {
        ...
        // 2. Use srcdoc (Modern way to inject HTML into an iframe)
        iframe.srcdoc = htmlContent;

        // 3. Wait for the iframe to finish loading content/styles
        iframe.onload = () => {
            iframe.contentWindow.focus();
            iframe.contentWindow.print();

            // 4. Cleanup: remove the iframe after the print dialog closes
            setTimeout(() => {
    ...
    render() {
        ...
        return html`
            <div id="header">
                ...
                <div id="buttons">
                    <button class="blue-button" @click=${this.openInDialogue}>
                    ...
                    Share</button>
                    <button class="blue-button" @click=${this.downloadHTML}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
                        <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5"/>
                        <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708z"/>
                    </svg><br>    
                    HTML</button>
                    <button class="blue-button" @click=${this.exportPdf}>
