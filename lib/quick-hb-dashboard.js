...
export class QuickHbDashboard extends LitElement {
  ...
  async updateStatus() {
    ...
    this.message = this._database ? `Spintax-Datenbank geladen ${new Date().toLocaleString('de-DE')}` : 'Keine Datenbank geladen';
  ...
  render() {
    return html`
      <div id="header">
        ...
        <div id="buttons">
          <fetch-button url="./data/db.zip" @database-changed="${this._databaseChanged}"></fetch-button>
        </div>
      </div>
